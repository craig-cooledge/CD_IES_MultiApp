...
def app():
    ...
    hist_df = pd.read_csv(os.path.join('ModelResults', 'KhKv_Histogram_Data.csv'))

    zone_select = st.sidebar.selectbox('Select a K Zone:', zone_list)
    plot_zone = 'Zone {0:0=3d}'.format(int(zone_select[0:3]))
    
    st.title('KhKv summary for K Zone: {0}'.format(zone_select))
    
    fig = px.histogram(hist_df, 
                       x= hist_df[plot_zone],
                       color = 'Series',
                       barmode = 'overlay',
                       title = 'KhKv Distribution for {0}'.format(plot_zone))
    ...
    in_csv = './ModelResults/IES_Nodal_KhKv.csv'
    ...
    with row2_1:
        ...
        box_fig = go.Figure()
        y0 = box_df['KhKv_fromMS']
        y1 = box_df['KhKv_Calibration']
        y2 = box_df['KhKv_Base']
        y3 = box_df['KhKv_IES_Avg']
        points = False
        ...
        box_fig.update_layout(title = 'Nodal Kh Distribution', yaxis = dict(showexponent = 'all', exponentformat = 'E', title = 'KhKv'))
        st.plotly_chart(box_fig, use_container_width = True)
    
    stat_df = box_df[['KhKv_fromMS','KhKv_Calibration', 'KhKv_Base', 'KhKv_IES_Avg']]
